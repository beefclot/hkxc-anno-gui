...
function App() {
  ...
  const [format, setFormat] = useState<"amd64" | "win32" | "xml">("amd64");
  ...
  const handleSelectFile = async () => {
    try {
      const selected = await open({
        multiple: true,
        filters: [{ name: "HKX Files", extensions: ["hkx", "xml"] }],
      });
  ...
  const handleEditorChange = (value: string | undefined) => {
    ...
    setTabs((prev) =>
      prev.map((tab) => (tab.annoPath === activeTabId ? { ...tab, content: value, modified: true } : tab))
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTabs((prev) => prev.map((tab) => (tab.annoPath === activeTabId ? { ...tab, modified: false } : tab)));
    ...
    <div className={`app ${isDragging ? "dragging" : ""}`}>
      <div className="header">
        ...
        <select
          ...
          value={format}
          onChange={(e) => setFormat(e.target.value as "amd64" | "win32" | "xml")}
        >
          <option value="amd64">64-bit (SE/AE)</option>
          <option value="win32">32-bit (LE)</option>
          <option value="xml">XML</option>
        </select>
        <button className="btn btn-success" onClick={handleSave} disabled={!activeTab || !activeTab.modified}>
          Save (Ctrl+S)
        </button>
        <button
        ...
        <button className="btn btn-help" onClick={() => setShowHotkeys(true)} title="Keyboard Shortcuts">
          ?
        </button>
      </div>
      ...
      {showSearch && (
        <div className="search-panel">
          ...
          {searchMode === "replace" && searchQuery && (
            <button className="btn btn-replace" onClick={handleReplaceAll} disabled={!searchQuery.trim()}>
                ...
                <div key={result.tabId} className="search-result-file">
                  ...
                  <div className="search-result-matches">
                    {result.matches.slice(0, 5).map((match, idx) => (
                      <div key={idx} className="search-match" onClick={() => setActiveTabId(result.tabId)}>
                        <span className="match-line">Line {match.line}:</span> {match.text}
                      </div>
                    ))}
                    {result.matches.length > 5 && (
                      <div className="search-match-more">...and {result.matches.length - 5} more</div>
                    )}
                  </div>
                </div>
              ))}
          ...
          {searchQuery && searchResults.length === 0 && <div className="search-no-results">No results found</div>}
        </div>
      )}
      ...
      <div className="editor-container">
        ...
        {activeTab ? (
          ...
          <div className="welcome">
            ...
            <p>
              <strong>Or drag & drop files/folders here</strong>
            </p>
            <div>
              <span className="kbd">Ctrl+S</span> Save current file •<span className="kbd">Ctrl+Shift+S</span> Save all
              modified files
            </div>
          </div>
        )}
        ...
        <div className="modal-overlay" onClick={() => setShowHotkeys(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowHotkeys(false)}>
                ×
              </button>
            </div>
              ...
              <table className="hotkeys-table">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>Find & replace in all files</td>
                  </tr>
                  <tr>
                    <td>
                      <kbd>Ctrl</kbd>+<kbd>F</kbd>
                    </td>
                    <td>Find in current file</td>
                  </tr>
                  <tr>
                    <td>
                      <kbd>Ctrl</kbd>+<kbd>H</kbd>
                    </td>
                    <td>Find & replace in current file</td>
                  </tr>
                  <tr>
                    <td>
                      <kbd>Escape</kbd>
                    </td>
                    <td>Close search panel / modal</td>
                  </tr>
                </tbody>
              </table>
            </div>
